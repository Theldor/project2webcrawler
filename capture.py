...
NAV_TIMEOUT = 45_000         # milliseconds – max wait per page load
WAIT_UNTIL = "domcontentloaded"  # "load" waits for all resources; often hangs on heavy sites (thangs, etc.)
ANALYZE_TIMEOUT = 15_000     # milliseconds – max for classifier (heavy pages can hang)
DELAY_BETWEEN_SITES = 3      # seconds – reduces rate limiting / network strain
OUTPUT_DIR = "dataset"       # root folder for all screenshots
SKIP_EXISTING = True         # skip URLs that already have a screenshot
MAX_RETRIES = 2              # retry failed captures up to N times
...
<<<<<<< HEAD
                ...
                if SKIP_EXISTING and os.path.exists(filepath):
                    skipped += 1
                    print(f"[{category}] ({idx}/{len(urls)}) {url} -> skipped (exists)")
                    continue
                ...
                for attempt in range(MAX_RETRIES + 1):
                    ...
                    page.set_default_timeout(ANALYZE_TIMEOUT)  # classifier can hang on heavy DOMs (e.g. Daily Mail)
                    auto_category = ""

                    try:
                        # domcontentloaded = DOM ready (fast); load = all resources (can hang on WebGL/heavy sites)
                        ...
                        if attempt < MAX_RETRIES:
                            wait = (attempt + 1) * 2
                            print(f"  !! attempt {attempt + 1} failed, retrying in {wait}s... ({exc})")
                            time.sleep(wait)
                        else:
                            failed += 1
                            print(f"  !! FAILED: {exc}")
